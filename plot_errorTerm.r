...
params_files = list.files('outputs/', pattern = params_files, full.names = TRUE)

prob_dir = "data/sampled_posterior/attempt15/control/"
prob_files = "Prob_info.csv"
...
#GFED_four_s_MDDM_GPCC/
prob_files = paste0(list.dirs(prob_dir), '/',prob_files) 
...
open_dr <- function(drv) {
    ...
    openDataset <- function(ba, pr) {
        ...
        grabError <- function(file) {
            x = read.csv(file)
            if (any(colnames(x) == 'sigma')) out = x[,'sigma'] 
            else out = x[,'P.given.TC'] 
        }    
...
errorTermFun <- function(fig_name, xlab = '~sigma~') {
    ...
    png(paste0("figs/erro_dist-", fig_name, ".png"),
            res= 300, units = 'in', height = 7.2, width = 7.2)
        par(mfrow = c(2, 2), mar = rep(0.5, 4), oma = c(3, 0, 0, 0)) 
        mapply(plot_dr, dats, names(dr_vars), mns, xaxs = c(F, F, T, T))
        mtext.units(side = 1, line = 2, xlab, outer = TRUE)
...
bins = seq(0.1, 0.2, length.out = 1001)
files = prob_files
errorTermFun('PbGy', 'P(~beta~|TC)')
