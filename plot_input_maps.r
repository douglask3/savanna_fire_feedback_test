...
pr_limits      = c(0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000)
...
drt_colour = c('#ffffd9', '#c7e9b4', '#41b6c4', '#225ea8', '#081d58', '#020d3a')
...
plot_vars =  paste0(c("MAP", drought_vars), '_', rep(pr_datasets, each = 5))

names(plot_vars) = paste(LETTERS[1:16], '', c("MAP", drought_vars),rep(pr_datasets, each = 5))


units   = c(rep('mm/yr', 4), rep('', 20))

scaling = c(rep(c(exp), 5), rep(1, 20))

limits = c(rep(list(pr_limits), 5), rep(list(drought_limits), 5))

col = c(rep(list(prc_colour), 5), rep(list(drt_colour), 20))
