...
stop()
plotSite <- function(siteID, pfile, global = TRUE) {
    dev.new()
    ps = read.csv(pfile)
    ...
    addMod <- function(p) {
        ...
        climLine <- function(climM, col = '#0000FF01') {
            yclim = logistic(xmod, climM, -p['climSigma'])
            lines(xp, yclim, col = make.transparent(col, 0.9), lwd = 2)
        }
    
        if (is.na(siteID)) {
            cols = c('#d73027','#f46d43','#fdae61',
                     '#e0f3f8','#abd9e9','#74add1','#4575b4')
            cols = make_col_vector(cols, ncols = length(climMean))
            mapply(climLine, sort(climMean), cols)
        } else ylim = climLine(climMean)
        ...
        if (!global) 1-sqrt(sum(ybio*ybio*yclim))/sqrt(sum(ybio*ybio))
    }
    ...
    mtext(side = 1, line = -7, adj = 0.1, paste0(climImpact[1], '-', climImpact[2], '%'),
          col = 'blue')    
...
if (doGlobal && F) {
    png("figs/globalSites.png", res = 300, units = 'in', width = 5, height =  5)
        plotSite(NaN, outGlobal)
    dev.off()
}
...
mapply(plotSite, siteIDs, outGlobal)
